...
class CNN:
    ...
    def compute_loss(self, y_pred, y_true):
        ...
        correct_log_probs = -np.log(p[np.arange(m), y_true])
    ...
    def train(self, dataset, num_epochs, batch_size, learning_rate):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(num_batches):
                ...
                gradient = y_pred - y_batch_onehot
                
                self.backward(gradient, learning_rate)

                print(f"Batch {i+1}/{num_batches}, Loss: {epoch_loss:.4f}, Accuracy: {batch_accuracy:.4f}, Time: {(time.time() - initial_time):.2f}s")
